...
export default async function GlossaryPage({ params }) {
  ...
  try {
    ...
    articles = data.filter((article) =>
      (article.glossary || []).some((item) => item.term.toLowerCase() === decodedTerm.toLowerCase())
    );
  ...
  return <GlossaryContent term={decodeURIComponent(term)} articles={articles} error={error} />;
}